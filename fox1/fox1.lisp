...
(defun parse-frame (buffer)
  (let ((frame (make-instance 'frame)))
    (setf (image-format frame)
          (ecase (readu16-le buffer)
            (1 :8-bit) (2 :bgr) (3 :bgra) (7 :bgra-recol)))
    (unless (eq (image-format frame) :8-bit)
      (error "Unsupported image format: ~A" (image-format frame)))
    ...
    (let* ((image (make-octet-vector (readu32-le buffer))))
      (fast-read-sequence image buffer)
